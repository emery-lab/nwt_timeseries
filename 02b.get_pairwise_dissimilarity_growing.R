...
sn.05s = sn.05s[, -which(names(sn.05s) %in% c("season"))]
...
sn.05s = merge(sn.05s, time.df, by.x = "date", by.y = "dates", all.x = TRUE)
...
sn.05s = sn.05s[, -which(names(sn.05s) %in% c("date"))]
...
rm(seq.05, seq.05s, sn.05, sn.05s)

#### 4. ADD TO PSI DATAFRAME ####
source("functions.R")
source("02a.get_pairwise_dissimilarity_all.R")

all.psi = merge.psi(psi.04.all, psi.05)
colnames(all.psi) = c("A", "B", "psi.all", "psi.all.scaled", "psi.growing")
all.psi = merge.psi(all.psi, psi.05s)
colnames(all.psi) = colnames(all.psi) = c("A", "B", "psi.all", "psi.all.scaled", "psi.growing", "psi.growing.scaled")
rm(psi.04.all, psi.05, psi.05s)
